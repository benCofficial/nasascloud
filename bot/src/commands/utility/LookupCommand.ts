        ...
        try {
            ...
            for (const user of users) {
                const embed = new Embed()
                    .setDescription(`UID ${user.uid} | [${user.username}](https://nasas.cloud/u/${user.uid}) (${user.blacklisted.status ? "Blacklisted" : ( this.client.owners.includes(user.discord.id) ? "Owner" :  (user.admin ? 'Admin' : (user.premium ? 'Premium' : 'Whitelisted')))})`)
